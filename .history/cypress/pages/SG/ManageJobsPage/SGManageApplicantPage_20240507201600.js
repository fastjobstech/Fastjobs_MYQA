class SGManageApplicantPage {
	...
	checkApplicantIsReceived = () => {
		cy
			.get(".job-app-card")
			.should("be.visible")
			.then(($applicantCardEl) => {
				const applicantEl = $applicantCardEl.find(".app-folder-link");

				if (applicantEl.length < 0) {
					cy.log("Application data still not been received!");
					cy.wait(10000);
					cy.reload();
				} else {
