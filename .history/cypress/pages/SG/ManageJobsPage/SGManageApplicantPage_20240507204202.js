class SGManageApplicantPage {
	...
	checkApplicantIsReceived = () => {
		cy.get(".job-app-card").find("#job-candidates > .panel > .panel-body");
		cy.get(".job-app-card").then(($applicantCardEl) => {
			const applicantEl = $applicantCardEl.find(
				"#job-candidates > .panel > .panel-body"
			);

			cy.wait(500);
			cy.log(applicantEl);
			if (applicantEl > 0 && applicantEl.is(":visible")) {
				cy.log("Application data haven't received!");
				cy.wait(10000);
				cy.reload();
			} else {
				cy.log("Application data is Received!");
			}
		});
	};
}
