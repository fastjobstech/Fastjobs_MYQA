class SGJobPostPage {
  elements = {
    ...
    JobListingTitle: () =>
      cy.get(".page-title > .pull-left", { timeout: 10000 }),
    ...
    // Packages Default
    PackageSelection: () =>
      cy.get(":nth-child(22) > .col-xs-2 > center > label"),
  ...
  VerifyJobPostingFeedbackModal = () => {
    cy.get("body").then(($el) => {
      ...
      if (
        feedbackModalElement.length > 0 &&
        feedbackModalElement.is(":visible")
      ) {
        cy.log("Feedback Rating modal is visible");
        //Submits feedback or close the modal?
        cy.get("#rating5").click();
        cy.get(".rating-comments > textarea").type(
