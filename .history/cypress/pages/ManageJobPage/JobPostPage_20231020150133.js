class JobPostPage {
    ...
    SubmitRatingModal = () => {
        this.elements.RatingModal()
            .should('be.visible')
            .then(() => {
                cy.get('.rating-scale > :nth-child(5)').click()
                cy.get('.rating-comments > textarea').type("This is a text from automated testing!")
                cy.get('.rating-submit').click()
                cy.get('#ratingSuccessModal > .modal-close').click()
            })
        // if (this.elements.RatingModal().should('be.visible')){
        //     cy.get('.rating-scale > :nth-child(5)').click()
        //     cy.get('.rating-comments > textarea').type("This is a text from automated testing!")
        //     cy.get('.rating-submit').click()
        //     cy.get('#ratingSuccessModal > .modal-close').click()
